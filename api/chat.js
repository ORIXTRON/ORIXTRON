...
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        // MODEL BARU SESUAI DEPRECATION HISTORY AGUSTUS 2026
        // Menggunakan Qwen 3.6 27B yang sangat baik untuk logika & finansial
        model: "qwen/qwen3.6-27b", 
        
        messages: messages,
        temperature: 0.4, // Tetap rendah agar jawaban faktual
        max_tokens: 1024
      })
    });
